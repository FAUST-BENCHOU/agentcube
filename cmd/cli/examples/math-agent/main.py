...
def initialize_agent():
    ...
    try:
        ...
        llm = ChatOpenAI(
            model=model_name, base_url=api_base_url, api_key=api_key, temperature=0.1
        )
...
def main():
    ...
    print(
        f"Starting Math Agent on port {port} at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
