...
class ControlPlaneClient:
    ...
    def __init__(
        ...
        if not self.base_url:
            raise ValueError(
                "Workload Manager URL must be provided via 'workload_manager_url' argument or 'WORKLOAD_MANAGER_URL' environment variable."
            )
    ...
    def create_session(
        ...
        try:
            ...
            if "sessionId" not in data or not data["sessionId"]:
                self.logger.error(
                    "Response JSON missing 'sessionId' in create_session response."
                )
                self.logger.debug(f"Full response data: {data}")
                raise ValueError(
                    "Failed to create session: 'sessionId' missing from response"
                )
