...
def start_picod_container():
    ...
    cmd = [
        ...
        f"{HOST_PORT}:8080",
        "-v",
        f"{BOOTSTRAP_KEY_FILE}:/etc/picod/public-key.pem",
        IMAGE_NAME,
        "-bootstrap-key",
        "/etc/picod/public-key.pem",
    ]
...
def perform_init_handshake(bootstrap_priv_key, session_pub_pem):
    ...
    token = jwt.encode(payload=claims, key=bootstrap_priv_key, algorithm="RS256")
...
def main():
    try:
        ...
        try:
            ...
            assert (
                False
            ), "ControlPlaneClient should have raised ValueError without WORKLOAD_MANAGER_URL"
